...
//
//$query = query('SELECT * FROM 202_sort_text_ads LEFT JOIN 202_text_ads USING (text_ad_id)', $db_table, false, false, false, $_values['order'], false, false, true);
...
while ($text_ad_row = $text_ad_result->getNext()) {
	...
	if (!$text_ad_row['text_ad_name']) {
		$text_ad_row['text_ad_name'] = '[no text ad recorded]';
	}

	echo
					$text_ad_row['text_ad_name'] . "\t" .
					$text_ad_row['sort_text_ad_clicks'] . "\t" .
					$text_ad_row['sort_text_ad_leads'] . "\t" .
					$text_ad_row['sort_text_ad_su_ratio'] . '%' . "\t" .
					dollar_format($text_ad_row['sort_text_ad_payout']) . "\t" .
					dollar_format($text_ad_row['sort_text_ad_epc']) . "\t" .
					dollar_format($text_ad_row['sort_text_ad_avg_cpc'], $cpv) . "\t" .
					dollar_format($text_ad_row['sort_text_ad_income']) . "\t" .
					dollar_format($text_ad_row['sort_text_ad_cost'], $cpv) . "\t" .
					dollar_format($text_ad_row['sort_text_ad_net'], $cpv) . "\t" .
					$text_ad_row['sort_text_ad_roi'] . '%' . "\n";
